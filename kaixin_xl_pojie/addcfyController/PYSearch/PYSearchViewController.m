...
#define CFYFile(path) @"/Library/PreferenceLoader/Preferences/cfyxunlei/PYSearch/" #path
...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.didSelectCellBlock = ^(UITableViewCell *didSelectCell) {
            __strong __typeof(_weakSelf) _swSelf = _weakSelf;
...
- (UIButton *)emptyButton
{
    if (!_emptyButton) {
        ...
        //[UIImage imageWithContentsOfFile:CFYFile(empty.png)]
        //
        [emptyButton setImage:[UIImage imageWithContentsOfFile:CFYFile(empty@3x.png)] forState:UIControlStateNormal];
...
- (UILabel *)searchHistoryHeader
{
    if (!_searchHistoryHeader) {
        UILabel *titleLabel = [self setupTitleLabel:@"搜索历史"];//[NSBundle py_localizedStringForKey:PYSearchSearchHistoryText]
...
- (void)setup
{
    ...
    [cancleButton setTitle:@"取消" forState:UIControlStateNormal];//[NSBundle py_localizedStringForKey:PYSearchCancelButtonText]
    ...
    UIImage *backImage = [UIImage imageWithContentsOfFile:CFYFile(back@2x.png)];
    ...
    searchBar.backgroundImage = [UIImage imageWithContentsOfFile:CFYFile(clearImage@2x.png)];
    ...
    UILabel *titleLabel = [self setupTitleLabel:@"热门资源"];//[NSBundle py_localizedStringForKey:PYSearchHotSearchText]
    ...
    emptySearchHistoryLabel.text = @"清空搜索历史";//[NSBundle py_localizedStringForKey:PYSearchEmptySearchHistoryText]
...
- (void)setupHotSearchRectangleTags
{
    ...
    for (int i = 0; i < PYRectangleTagMaxCol - 1; i++) {
        UIImageView *verticalLine = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:CFYFile(cell-content-line-vertical@3x.png)]];
    ...
    for (int i = 0; i < ceil(((double)self.hotSearches.count / PYRectangleTagMaxCol)) - 1; i++) {
        UIImageView *verticalLine = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:CFYFile(cell-content-line@3x.png)]];
...
- (void)setupHotSearchRankTags
{
    ...
    for (int i = 0; i < self.hotSearches.count; i++) {
        ...
        UIImageView *line = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:CFYFile(cell-content-line@3x.png)]];
        line.py_height = 0.5;
        line.alpha = 0.7;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        [closetButton setImage:[UIImage imageWithContentsOfFile:CFYFile(close@2x.png)] forState:UIControlStateNormal];
        UIImageView *closeView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:CFYFile(close@2x.png)]];
    ...
    cell.imageView.image = [UIImage imageWithContentsOfFile:CFYFile(search_history@2x.png)];
