...
/** 播放状态发生了改变的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nullable SPVideoPlayerStateChangedNSNotification;
/** 进度值发生了改变的通知名称 */
/** 当在快进快退的时候，视频的播放进度并未发生真实的改变(仍然处于自然播放状态)，只有快进快退结束时，视频播放进度才开始发生真实跳转 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerProgressValueChangedNSNotification;
/** 将要跳转到哪一时间进行播放的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerWillJumpNSNotification;
/** 跳转完毕的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerDidJumpedNSNotification;
/** 缓冲进度发生了改变的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerBufferProgressValueChangedNSNotification;
/** 视频截图完成了的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerCutVideoFinishedNSNotification;
/** 媒体网络加载状态发生了改变的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerLoadStatusDidChangedNotification;
/** 亮度或音量发生了改变的通知名称 */
UIKIT_EXTERN NSNotificationName const _Nonnull SPVideoPlayerBrightnessOrVolumeDidChangedNotification;
...
/** 下载视频,返回下载路径 */
- (NSString *_Nonnull)sp_playerDownload:(NSString *_Nonnull)url;
/** 控制层即将显示 */
- (void)sp_playerControlViewWillShow:(UIView *_Nonnull)controlView isFullscreen:(BOOL)fullscreen;
/** 控制层即将隐藏 */
- (void)sp_playerControlViewWillHidden:(UIView *_Nonnull)controlView isFullscreen:(BOOL)fullscreen;
...
/** 设置代理 */
@property (nonatomic, weak) id<SPVideoPlayerDelegate> _Nullable      delegate;
/** 是否被用户暂停 */
 ...
 *  @return SPPlayer
 */
+ (instancetype _Nonnull)sharedPlayerView;
 ...
 * 此方法的主要作用有3个：
    ...
    3、获取播放资源
 */
- (void)configureControlView:(UIView *_Nonnull)controlView videoItem:(SPVideoItem *_Nonnull)videoItem;

/**
 *  相当于上面那个方法的controlView传nil
 */
- (void)configureVideoItem:(SPVideoItem *_Nonnull)videoItem;
 ...
 * 此方法的主要作用有3个：
    ...
    3、获取播放资源
 */
- (void)configureControlView:(UIView *_Nonnull)controlView videoItems:(NSArray<SPVideoItem *> *_Nonnull)videoItems;
 ...
 *  在当前页面，设置新的视频时候调用此方法
 */
- (void)resetToPlayNewVideo:(SPVideoItem *_Nonnull)videoItem;
